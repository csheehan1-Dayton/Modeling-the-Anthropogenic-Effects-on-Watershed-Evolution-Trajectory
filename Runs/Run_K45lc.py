...
project_directory = script_directory.replace('\\Runs\\Run_K45lc', '')
...
m_sp = 0.503                         
n_sp = 1.224                         
# K = 8.29124436168032E-07
D_star = 262.5
K_ratio_end = 1.084426617
...
# Create K timeseries
print('Creating K timeseries...')
K_ratio_array = np.arange(1, K_ratio_end, (K_ratio_end - 1) / (np.size(t)) )
...
for ti in t:
    ...
    spr = StreamPowerEroder(mg, 
                            K_sp = K * K_ratio_array[timestep_integer], 
                            m_sp = m_sp, 
                            n_sp = n_sp, 
    ...
    D = K * K_ratio_array[timestep_integer] * D_star
