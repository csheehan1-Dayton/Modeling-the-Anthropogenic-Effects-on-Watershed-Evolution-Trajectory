...
# Import K 
K_sp_path = project_directory + '\K1_create\K1_create_output\K1h.csv'
K_sp = pd.read_csv(K_sp_path, header = None)
K_sp = K_sp.values
K_sp = np.squeeze(K_sp)

# Import K_star
K_star_path = project_directory + '\K_star\pd_f_c_calibration_decadal_output\K_star_45h.csv'
K_star = pd.read_csv(K_star_path, header = None)
K_star = K_star.values
...
# Export parameters
plot_interval = 10          # years
export_DEM_interval = 10     # years
export_format = 'png'
dpi = 300 
...
for ti in t:
    ...
    # Renitialize StreamPowerEroder and LinearDiffuser
    spr = StreamPowerEroder(mg, K_sp = K * K_star[timestep_integer], m_sp = m_sp, n_sp = n_sp, erode_flooded_nodes = True)
    D = K * K_star[timestep_integer] * D_star
